...
| 8 | 两类导入业务预检规则 | 第 8 节 | confirmed | 2026-09-13 用户确认通过；采用 POP 当前值按提交顺序覆盖规则；同条码新供应商 B 覆盖 A 并只保留 B |
...
| `supplierName` | 门店商品当前供应来源 | `WHEN_PURCHASE_PRICE_PRESENT` | 新名称无需预建，非空时替换旧供应商；空值保留当前供应商 |
...
| `supplierName` | 门店商品当前供应来源及当前进价定位 | `WHEN_PURCHASE_PRICE_PRESENT` | 新名称无需预建，非空时替换旧供应商；不进入销售分析维度 |
...
必须先过滤零销量行，再做同条码聚合。被过滤行不参与销售、进价、售价或库存取值。同一批次相同条码的供应商和对应进价作为一组，按 Excel 原始行号最后一行覆盖；若胜出行供应商非空但进价为空，则新供应商的当前进价为空，不得沿用旧供应商的进价。售价和库存存在多个非空值时仍分别取最后一个非空值。上述覆盖均在预检中提示，不作为错误。
...
- 每个目标门店商品只保留一个当前供应商，当前进价随“目标门店 + 商品 + 当前供应商”组合保存。文件中出现非空新供应商时无需预建，提交时用新供应商及其配套进价替换旧供应商记录；商品未出现或供应商为空时保留当前供应商。
- 同一文件内相同条码出现不同供应商时，按 Excel 原始行号最后一行胜出，提交后只保留该供应商；其他多行非空当前值也按 Excel 原始行号最后一行胜出。跨批次以后成功提交的批次胜出，`sourceDate` 只用于追溯。
...
- 过滤后同条码多供应商行按条码聚合销售数量、收入等销售事实；供应商及配套当前进价按 Excel 原始行号最后一行覆盖，提交后门店商品只保留该供应商。供应商不作为销售事实维度。
- 同条码库存、门店售价存在多个非空值时，按 Excel 原始行号取最后一个非空值并给出提示，不阻断批次。
- 同条码当前供应商或配套进价存在多个非空值时，取 Excel 原始行号最后一行并给出提示；进价有值但供应商名称为空时仍为错误。非空的新供应商名称无需预建或识别为既有主档，提交时直接替换旧供应商。
...
| 重复条码出现供应商 A、B | 销售按条码聚合；按原始行号最后一行由 B 覆盖 A，提交后该门店商品只保留 B 及其配套进价 |
| 重复条码库存/售价多值 | 分别取 Excel 最后一条非空值，给出提示但不阻断 |
| 胜出供应商行进价为空 | 保存新供应商且当前进价为空，不沿用旧供应商进价 |
