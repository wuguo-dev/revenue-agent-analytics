...
API-035 修改条码时，新条码必须非空、格式合法且全局唯一；冲突返回 409 `PRODUCT_BARCODE_CONFLICT`，并发版本不一致返回 409 `VERSION_CONFLICT`。修改只变更该商品的当前业务条码，内部商品 ID 不变，因此历史销售、门店商品、库存快照、供应商和货位关系继续归属同一商品；同时创建 ES 索引同步任务。商品资料导入仍以条码作为匹配键，不能通过同一行表达“旧条码改成新条码”。普通用户没有商品写接口，调用 API-034 至 API-038 均返回 403。条码修改必须记录操作人、修改时间、旧条码和新条码；旧条码是否作为导入别名继续识别仍为 G1 待确认项。
...
2. 商品条形码已确认仅管理员可修改；仍需确认最大长度、纯数字规则，以及修改后旧条码是否继续作为导入匹配别名。
3. POP 两类表格的最终字段、表头别名、数量和金额精度。
4. 销售汇总中退货、销售额、成本、毛利和毛利率的确切统计公式。
5. 库存超过多少天未被两类 POP 文件覆盖时标记为陈旧。
6. Agent 单次运行超时、并发限制、会话保留期限。
7. 审计日志和原始导入文件的保留期限。
