...
M02 是账号、固定角色、门店和账号门店绑定的权威模块；M01 负责认证与全部会话撤销，M10 消费脱敏审计事件，其他业务模块通过不暴露内部表结构的门店引用探针参与物理删除检查。任一引用检查不可用或不完整时删除失败。所有 `/admin/**` 接口由后端强制校验 `ADMIN`；API-019 的目标只取当前认证主体，不能提交其他用户 ID。写操作复用 M01 的 CSRF Token 与 Origin 校验并采用请求字段白名单，密码明文、密码哈希、会话标识、完整请求体和内部服务端字段不得进入响应、日志或审计。
