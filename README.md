...
- 商品—供应商关系只保存最新进价，不建设历次进价历史；历史销售成本以每日销售事实快照为准，普通用户不能查看进价。
...
- 商品资料导入前由管理员必选资料日期，作为该批次门店售价和库存的来源日期；旧资料不能回退较新的售价或库存。
- 已有商品采用非空覆盖：空单元格保留旧值，有效非空值才覆盖；空值不表示清空或写零。
- 商品资料和每日销售文件中的有效库存值直接覆盖对应门店商品库存，不使用销量推算库存。
- 每日销售文件中“本期销售数量”为 0 的行整行舍弃，不创建销售事实，也不更新门店售价或库存；负销量仍按退货事实处理。
