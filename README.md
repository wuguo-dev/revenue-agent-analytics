...
- 商品通过商品资料首次进入本系统时，使用该行 POP 商品类别初始化其分类；已有商品在后续商品资料或每日销售导入中都不再自动修改分类。
- 管理员可在商品页面手工调整已有商品分类；系统不保存“最近一次 POP 分类”，也不提供恢复 POP 分类操作。
- 分类停用不级联停用或移动商品；已有商品继续显示原分类及停用标记，但该分类不能再用于新的商品归属。
...
- 商品—供应商关系只保存最新进价，不建设历次进价历史；商品资料和每日销售汇总都可按来源日期更新当前进价，历史销售成本仍以每日销售事实快照为准，普通用户不能查看进价。
